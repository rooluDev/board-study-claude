...
- **from**: 미입력 시 1년 전
- **to**: 미입력 시 오늘
- **keyword**: 선택사항 (미입력 시 날짜 범위와 카테고리로만 검색)
- **category**: 미입력 시 전체
...
public class SearchParams {
    ...
    private LocalDate from; // 기본값: LocalDate.now().minusYears(1)
    private LocalDate to;   // 기본값: LocalDate.now()
    private String keyword; // null 가능
}
```
...
public static void validateWriter(String writer) {
    ...
    if (length < 2 || length > 10) {
        throw new ValidationException("작성자는 2자 이상 10자 이하로 입력해주세요.");
    }
}
```
...
<select id="selectBoardList" resultType="Board">
  SELECT 
    ...
    b.edited_at,
    c.category_name,
    (SELECT COUNT(*) FROM file WHERE board_id = b.board_id) as file_count
  FROM board b
...
5. **파일 저장 경로**: 반드시 `./src/main/webapp/uploads` 사용
