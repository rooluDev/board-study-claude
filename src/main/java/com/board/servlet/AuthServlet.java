 ...
 * 비밀번호 확인 API 서블릿
 *
 * POST /auth/confirm: 게시글 비밀번호 확인
 */
@WebServlet("/auth/confirm")
public class AuthServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      // 성공 응답
      sendJsonResponse(response, true, "비밀번호가 확인되었습니다.");

    } catch (AuthenticationException e) {
      // 비밀번호 불일치
      sendJsonResponse(response, false, e.getUserMessage());
      ...
      // 예상치 못한 오류
      e.printStackTrace();
      sendJsonResponse(response, false, "시스템 오류가 발생했습니다.");
    }
  }
